...
for (i in unique(trt)){
    if(i != "Ctl"){
   comparison1 <- results(results, pAdjustMethod = "BH", contrast = c("trt",i,"Ctl"))
	 ...
	 write.table(YY, file = paste0(outname, ".", i,"_vs_Ctl.padjstat"), sep="\t", row.names = FALSE)
	 ZZ=cbind(rownames(dds), comparison1a$pvalue, comparison1a$TrueFC)
	 write.table(ZZ, file = paste0(outname, ".", i,"_vs_Ctl.pstat"), sep="\t", row.names = FALSE)
	 comparison1a <- cbind(rownames(dds),comparison1)
	 write.table(comparison1a, file = paste0(outname, ".", i, "_vs_Ctl.txt"), sep="\t", row.names = FALSE)
	 ...
	 print (paste0(filename, "|", outname, "|", i, "_vs_Ctl", "|", "p<=0.05:", sigN ,"|",  "padj<=0.05:", adjSigN,
